...
x = double(LP.V{1,k});                                                  % double precision
prePad = x(1)+zeros(1,100); postPad = x(end)+zeros(1,100);              % generate pads
x = [prePad,x,postPad];                                                 % pad vector
x = resample(x,5e4,round(double(1000/LP.acquireRes)));      % resample
if round(double(1000/LP.acquireRes)) == 1e4                           % remove pad 10kHz
    x = x(501:end-500); % length==315
elseif round(double(1000/LP.acquireRes)) == 2e4                       % remove pad 20 kHz
    x = x(251:end-250); % length==313
elseif round(double(1000/LP.acquireRes)) == 2e5                      % remove pad 200 kHz
