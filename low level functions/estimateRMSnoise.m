...
vec_pre = double(LP.V{1,k}(LP.stimOn-(500/LP.acquireRes)-1:LP.stimOn-1));
t_pre = LP.stimOn-(500/LP.acquireRes)-1:LP.stimOn-1;
vec_post = double(LP.V{1,k}(LP.stimOff+(250/LP.acquireRes):LP.stimOff+(750/LP.acquireRes)-1));
...
% long-term noise
LP.restVPre(1,k) = mean(vec_pre);
rmse_pre = sqrt(mean((vec_pre - LP.restVPre(1,k)).^2));
LP.restVPost(1,k) = mean(vec_post);
rmse_post = sqrt(mean((vec_post - LP.restVPost(1,k)).^2));

% short-term noise
stWin = 1.5/LP.acquireRes;
winCount = 1;
...
diffV_b_e = abs(LP.restVPre(1,k)-LP.restVPost(1,k)); % differnce between end and stim onset

clear vec_pre t_pre vec_post t_post stWin